...
export function EditWalletModal({ wallet, onEdit, isOpen, setIsOpen }: { wallet: any, onEdit: () => void, isOpen: boolean, setIsOpen: (val: boolean) => void }) {
    const [loading, setLoading] = useState(false);
    ...
    async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
        ...
        const formData = new FormData(e.currentTarget);
        const amount = Number(formData.get("amount"));
        const rupeesToPaise = amount * 100;

        const { updateWallet } = await import('@/app/actions/wallets');
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm px-4">
            <div className="bg-white dark:bg-gray-900 rounded-3xl w-full max-w-md shadow-2xl overflow-hidden">
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-6">
                    <div>
                        <label htmlFor="edit-wallet-name" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                            Account Name
                        </label>
                        <input
                            id="edit-wallet-name"
                    ...
                    <div>
                        <label htmlFor="edit-wallet-amount" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                            Starting Balance (₹)
                        </label>
                        <input
                            ...
                            type="number"
                            required
                            defaultValue={(wallet.openingBalance / 100).toString()}
                            step="0.01"
                            className="w-full border-gray-300 dark:border-gray-700 rounded-xl bg-gray-50 dark:bg-gray-800 px-4 py-3 focus:ring-[var(--color-brand-navy)] focus:border-[var(--color-brand-navy)] transition-colors"
                        />
                    </div>
                            ...
                            {colors.map((c) => (
                                <label key={c} className="relative cursor-pointer">
                                    ...
                                    <input
                                        ...
                                        className="peer sr-only"
                                        required
                                        defaultChecked={wallet.color === c || (!wallet.color && c === colors[0])}
                                    />
                                    <div
                                        className="w-8 h-8 rounded-full border-2 border-transparent peer-checked:ring-2 peer-checked:ring-offset-2 peer-checked:ring-[var(--color-brand-navy)] dark:peer-checked:ring-offset-gray-900 transition-all dynamic-bg"
                                        style={{ "--dynamic-color": c } as React.CSSProperties}
